<script lang="ts">
	import { stick } from './mouse/actions';

	export let name: string;
	export let description: string;
	export let price: number;
	export let features: string[];
</script>

<div
	class="flex flex-col rounded-xl border border-foreground/10 bg-white/40 p-10 text-foreground dark:bg-black/40"
	use:stick
>
	<div class="font-nelphim text-3xl font-bold">{name}</div>
	<div class="mt-3 font-roboto">{description}</div>
	<div class="mt-8 font-staal text-6xl">{price}$</div>
	<div class="my-5 h-px w-full border border-t border-foreground/10"></div>
	<div class="flex-1">
		{#each features as feature}
			<div class="flex items-center">
				<span class=" i-tabler-check" />
				<p class="pl-5 font-roboto">{feature}</p>
			</div>
		{/each}
	</div>
	<div class="mt-10">
		<button
			class="w-full rounded-md bg-foreground py-3 font-roboto text-background hover:bg-foreground/80"
			>Get Plan</button
		>
	</div>
</div>
